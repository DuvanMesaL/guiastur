...
if (strpos($requestUri, '/api/users/create') !== false) {
    require_once $_SERVER["DOCUMENT_ROOT"] . "/api/routes/Users/createUsers.php";
} elseif (strpos($requestUri, '/api/users/login') !== false) {
    error_log("Redirigiendo a login.php", 0);
    require_once $_SERVER["DOCUMENT_ROOT"] . "/api/routes/Users/login.php";
} elseif (strpos($requestUri, '/api/users/logout') !== false) {
    require_once $_SERVER["DOCUMENT_ROOT"] . "/api/routes/Users/logout.php";
} elseif (strpos($requestUri, '/api/users/refreshtoken') !== false) {
    require_once $_SERVER["DOCUMENT_ROOT"] . "/api/routes/Users/refreshtoken.php";
} elseif (strpos($requestUri, '/api/buques/create') !== false) {
    require_once $_SERVER["DOCUMENT_ROOT"] . "/api/routes/Buques/CreateBuque.php";
} else {
